...
public class BattingCalculation : MonoBehaviour
{
    ...
    [Header("タイミングがパーフェクトの時の倍率"),SerializeField] private float _perfectMultiplier;
    [Header("タイミングがグッドの時の倍率"),SerializeField] private float _goodMultiplier;
    [Header("タイミングがフェアの時の倍率"),SerializeField] private float _fairMultiplier;
    ...
    public class BattingTypeParameters
    {
        ...
        [Header("ミートの範囲倍率")] public float ContactRangeBonus;  // ミート範囲
    }

    public float CalculatePositionBasedTiming(Vector3 ballPosition, Vector3 batCore)
    {
    ...
    public Vector3 CalculateBattingDirection(Vector3 ballPos, Vector3 batCore, float timing, BattingType type)
    {
        ...
        Vector3 battingDirection = new Vector3(
            (xDistance + p.DirectionXModifier) * _battingXMultiplier,
            timing * _maxHihgt * p.DirectionYModifier + yDistance * _battingYMultiplier,
